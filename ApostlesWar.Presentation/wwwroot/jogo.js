...
function medidorDoChaoDaVila(canvas) {
...
const linhaDaArvore = (medida, arvoreCfg) => medida.linha - medida.faixa * arvoreCfg.acima;
...
function tracarMembro(ctx, x0, y0, ang, comp, r0, r1, curva) {
...
const tremorDoChao = (inferno, canvas, t, escala) => {
...
function curvaPelosPontos(ctx, pontos) {
...
/// eles se sobrepõem.
function tracarFitaDeRaiz(ctx, pontos, r0, r1) {
...
function criarArvoreDoMundo(cfg, canvas, inferno) {
...
function criarVeias(cfg, canvas, arvoreCfg, inferno) {
    let t = 0;
...
function criarBuracoDoInferno(cfg, canvas, arvoreCfg, inferno) {
    const medir = medidorDoChaoDaVila(canvas);
    let fase = 'dormindo';
    let relogio = 0;
    let q = 0;
...
function criarRespingosDoInferno(cfg, canvas, inferno) {
    return (ctx) => {
...
function criarFumacaDoInferno(cfg, canvas, arvoreCfg, inferno) {
    const medir = medidorDoChaoDaVila(canvas);
    let t = 0;
...
const criarFogosDaVila = (cfg, canvas) => {
    let t = 0;
...
function desenharChama(ctx, x, base, alt, t, cfg) {
    ctx.save();
...
function medidorDoChaoDaSala(canvas) {
...
const duracaoDoPasso = (cfg, passo) => (passo === 'subir' ? cfg.descer : cfg[passo]);

function criarRoteiroDaNoite(cfg, canvas, natal) {
    let i = 0;
...
function desenharPinheiroNevado(ctx, x, chao, alt, larg, cor, neve) {
    ctx.fillStyle = cor;
    ctx.beginPath();
...
const caixaDaJanela = (cfg, canvas) => {
...
function criarVistaDaJanela(cfg, canvas, natal) {
...
function criarCortinas(cfg, canvas, janelaCfg) {
    return (ctx, dt) => {
...
function caminhoDaBoca(ctx, cx, topo, largura, fundo) {
    ctx.beginPath();
...
function criarLareira(cfg, canvas, natal) {
...
function desenharMascaraTeatro(ctx, cx, cy, s, cor, sorri, giro, traco) {
    ctx.save();
    ctx.translate(cx, cy);
    ctx.rotate(giro);
...
function criarArvoreDeNatal(cfg, canvas, natal) {
    const medirChao = medidorDoChaoDaSala(canvas);
    let t = 0;
...
function criarPresentes(cfg, canvas, natal, arvoreCfg, lareiraCfg) {
    const medirChao = medidorDoChaoDaSala(canvas);
    let visto = 0;
    const pilha = [];
    const nova = (i) => ({
...
function desenharCaixa(ctx, x, y, s, cor, giro, cfg) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(giro);
...
function desenharEstrela(ctx, cx, cy, s, aceso, t, cfg) {
    ctx.save();
...
function desenharAnjinho(ctx, cx, cy, s, cfg) {
...
/// não existe aqui.
function desenharBonecoDeNeve(ctx, x, base, s, cfg) {
    ctx.beginPath();
...
function desenharTreno(ctx, x, y, s, angulo, fase, piscas, cfg) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angulo);
...
function desenharRena(ctx, x, y, s, fase, guia, cfg) {
    ctx.save();
    ctx.translate(x, y);
    ctx.fillStyle = cfg.corpo;
    ctx.strokeStyle = cfg.corpo;
