namespace ApostlesWar.View
{
    ...
    /// </summary>
    enum TipoComando { Cima, Baixo, Esquerda, Direita, Confirmar, Cancelar, Selecionar, Nenhum }

    /// <summary>Comando de navegação já semântico. `Numero` só é usado quando `Tipo == Selecionar`.</summary>
    readonly record struct Comando(TipoComando Tipo, int Numero = 0);

    /// <summary>
    /// Porta de ENTRADA — o par simétrico do <see cref="IApresentacao"/> (saída). Traduz o input cru
    /// num <see cref="Comando"/> semântico; quem consome dá switch na INTENÇÃO, não na tecla. No porte
    /// futuro (web/Unity) troca-se `EntradaConsole` por `EntradaWeb`/`EntradaUnity` — o resto não muda.
    /// Mouse (quando o alvo for escolhido) entra pelo `Selecionar(N)`, que já é "apontar direto".
    /// </summary>
    interface IEntrada
    {
    ...
    /// <summary>Implementação de console: lê tecla e mapeia pra <see cref="Comando"/>. Trocável (web/Unity/testes).</summary>
    internal class EntradaConsole : IEntrada
    {
        public Comando Ler()
        {
